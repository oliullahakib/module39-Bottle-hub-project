...
const Bottles = ({ bottlePromiss }) => {
    ...
    const handleCart = (bottle) => {
        ...
        setCart(newCart)
        alert(`${bottle.name} is Added To Cart`)
    }
    const handldeDeleteCart = (id) => {
       const filteredCart = cart.filter(bottle =>bottle.id !==id);
       setCart(filteredCart)
       deleteCartFromLs(id)
    }
    const deleteCartFromLs =(id)=>{
        const cartIdArr = getFromLs();
        if(cartIdArr){
           const filtaredCartIdArr = cartIdArr.filter(cartId => cartId !==id);
           const filterCartArrString = JSON.stringify(filtaredCartIdArr)
           localStorage.setItem("cartId",filterCartArrString)
        }
    }
    return (
        <div >
            <Carts cart={cart} handldeDeleteCart={handldeDeleteCart} />
